...
const PERSISTENCE_KEY_FORM = 'kp_v9_form_state';
const PERSISTENCE_KEY_STORY = 'kp_v9_story_state';

const App: React.FC = () => {
  const [formData, setFormData] = useState<StoryInput>(() => {
    const saved = localStorage.getItem(PERSISTENCE_KEY_FORM);
  ...
  const [storyboard, setStoryboard] = useState<Storyboard | null>(() => {
    const saved = localStorage.getItem(PERSISTENCE_KEY_STORY);
    try {
  ...
  useEffect(() => {
    localStorage.setItem(PERSISTENCE_KEY_FORM, JSON.stringify(formData));
  }, [formData]);

  useEffect(() => {
    if (storyboard) {
      localStorage.setItem(PERSISTENCE_KEY_STORY, JSON.stringify(storyboard));
    }
  }, [storyboard]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || "V9 Engine failed. Check API quota.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const copyShortNarration = (narration: string, id: number) => {
  ...
  const handleAddSceneManual = () => {
    ...
    const scene: Scene = {
        ...
        duration: 6,
        grok_prompt: `Master visual for segment ${nextNum}: ${manualScene.setting}. Stylized 3D Realism.`
    };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <section className="mb-20">
            <div className="max-w-4xl mx-auto">
                <div className="text-center mb-16">
                    <h2 className="text-7xl font-black text-transparent bg-clip-text bg-gradient-to-br from-indigo-400 via-purple-500 to-pink-500 mb-6 tracking-tighter filter drop-shadow-sm">Karakter Paten v9</h2>
                    <p className="text-slate-400 text-2xl font-medium tracking-tight max-w-2xl mx-auto leading-snug italic">V9 Intelligence Engine • PolyMatter Visual Flow</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="bg-slate-900/60 backdrop-blur-2xl p-12 rounded-[3.5rem] border border-slate-800 shadow-[0_20px_50px_rgba(0,0,0,0.5)] space-y-12">
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 hover:shadow-[0_0_50px_rgba(79,70,229,0.3)] active:scale-[0.98] text-white font-black py-7 rounded-[2.5rem] transition-all flex items-center justify-center gap-6 text-2xl uppercase tracking-tighter">
                        {loading ? "V9 INTELLIGENCE ENGINE PROCESSING..." : "GENERATE PRODUCTION PACKAGE V9"}
                    </button>
        ...
        {storyboard && (
            <div className="space-y-20 animate-fade-in">
                ...
                {storyboard.shorts && storyboard.shorts.length > 0 && (
                                ...
                                <div key={i} className="bg-slate-950 p-8 rounded-[2.5rem] border border-slate-800 group hover:border-pink-500/40 transition-all flex flex-col h-full shadow-lg">
                                    ...
                                    <div className="space-y-4 pt-6 border-t border-slate-800/50">
                                        <div className="bg-slate-900/50 p-4 rounded-2xl space-y-3">
                                            <span className="text-[9px] text-indigo-400 font-black uppercase tracking-widest block">Video Prod. Prompt</span>
                                            <button 
                                            ...
                                            >
                                                {copiedPromptId === short.id ? 'Copied Prompt!' : 'Copy Prompt'}
                                            </button>
                                            <button 
                                                onClick={() => copyShortNarration(short.narration, short.id)}
                                                className={`w-full py-2 rounded-xl text-[10px] font-black uppercase transition-all flex items-center justify-center gap-2 ${copiedNarrativeId === short.id ? 'bg-emerald-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white'}`}
                                            >
                                                {copiedNarrativeId === short.id ? 'Copied Script!' : 'Copy Script'}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="text-center md:text-left flex-1">
                        <div className="flex items-center gap-4 justify-center md:justify-start mb-4">
                            <span className={`px-4 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${storyboard.story_type === 'hybrid' ? 'bg-indigo-600 text-white shadow-[0_0_15px_rgba(79,70,229,0.4)]' : 'bg-pink-600 text-white shadow-[0_0_15px_rgba(236,72,153,0.4)]'}`}>
                                ENGINE v9 • INTELLIGENCE LAYER ACTIVE
                            </span>
                        </div>
